...
@section('content')
    <div class="page-content">
        ...
        <div class="col-12">
            <form action="{{ route('marks-register.search') }}" method="get" id="marksheet" class="exam_assign">
                            ...
                            <div class="single_large_selectBox">
                                <select id="getSections"
                                    ...
                                    @foreach ($data['classes'] as $item)
                                        <option
                                            {{ request('class')  ==  $item->class->id ? 'selected' : ''}} value="{{ $item->class->id }}">{{ $item->class->name }}</option>
                                    @endforeach
                                </select>
                                @error('class')
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered role-table">
                            ...
                            <tr>
                                ...
                                <th class="purchase">{{ ___('examination.Marksheet Published') }}</th>
                                <th class="purchase">{{ ___('examination.student') }} & {{ ___('examination.mark') }}
                                </th>
                            ...
                            @forelse ($data['marks_registers'] as $key => $row)
                                <tr id="row_{{ $row->id }}">
                                    ...
                                    <td id="marksheet-status-{{ $row->id }}">
                                        @if ($row->is_marksheet_published)
                                            <span class="badge-basic-success-text">Yes</span>
                                            @if (hasPermission('marks_register_delete'))
                                                <input type="checkbox" checked value="unpublish"
                                                       onchange="updateMarksheetStatus(event, {{ $row->id }})">
                                            @endif
                                        @else
                                            <span class="badge-basic-danger-text">No</span>
                                            @if (hasPermission('marks_register_delete'))
                                    ...
                                    <td>
                                        <a href="#" class="btn btn-sm ot-btn-primary" data-bs-toggle="modal"
                                           ...
                                           onclick="viewStudentMark({{ $row->id }})">
...
@push('script')
    ...
    <script>
        function updateMarksheetStatus(event, id) {
            const isChecked = event.target.checked;
            const action = isChecked ? 'publish' : 'unpublish';

            fetch(`/marks-register/update-marksheet-status/${id}`, {
                method: 'POST',
                headers: {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        ...
                        const statusCell = document.getElementById(`marksheet-status-${id}`);
                        const badge = statusCell.querySelector('span');
                        const newText = isChecked ? 'Yes' : 'No';
                        const newClass = isChecked ? 'badge-basic-success-text' : 'badge-basic-danger-text';
                        ...
                        Toast.fire({
                            icon: 'success',
                            title: 'Marksheet status updated successfully.'
                        })

                    } else {
                        Toast.fire({
                            icon: 'error',
                            title: 'Failed to update marksheet status.'
                        })
                    }
                })
                .catch(error => {
                    Toast.fire({
                        icon: 'error',
                        title: 'An error occurred while updating the marksheet status.'
                    })
                });
        }
    </script>
