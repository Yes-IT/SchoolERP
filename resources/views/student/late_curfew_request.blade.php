...
@section('content')
    ...
    <div class="ds-pr-body late-curfew-request-pg">
        ...
        <div class="atndnc-filter-wrp w-100">
            ...
            <div class="atndnc-filter">
                <form method="GET" action="{{ route('student.late_curfew_request') }}">
                    @csrf
            ...
            <div class="ds-content-head">
                ...
                <div class="ds-cmn-filter-wrp">
                    <div class="dsbdy-filter-wrp p-0">
                        <button class="cmn-btn h-40" data-bs-target="#addRequest" data-bs-toggle="modal"><i
                                class="fa-solid fa-plus"></i> Add Request
                        </button>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($requested as $index => $item)
                            <tr>
                                ...
                                <td>{{ $item->room }}</td>
                                <td>
                                    <div class="linecamped line-count-1" title="{{ $item->reason }}">
                                        {{ $item->reason ?? 'N/A' }}</div>
                                </td>
                                <td>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->status === 'approved')
                                        <p class="green-bg cmn-tbl-btn">Approved ({{ $item->updated_at->format('d/m/Y') }})
                                        </p>
                                ...
                                <td>
                                    @if ($item->status === 'approved')
                                        <div class="actions-wrp">
                                            <button type="button" class="edit-btn" data-id="{{ $item->id }}"
                                                data-bs-toggle="modal" data-bs-target="#editRequest">
                                                <img src="{{ asset('student/images/edit-icon-primary.svg') }}"
                                                    alt="Icon">
                                            </button>
    ...
    <div class="modal fade cmn-popwrp pop800" id="addRequest" tabindex="-1" role="dialog" aria-labelledby="addRequest"
        aria-hidden="true">
    ...
    <div class="modal fade cmn-popwrp pop800" id="editRequest" tabindex="-1" role="dialog"
        aria-labelledby="editRequest" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    ...
                    <div class="cmn-pop-inr-content-wrp">
                        ...
                        <div class="request-leave-form-wrp">
                            ...
                            <form method="POST" action="" id="editRequestForm"
                                data-base-route="{{ route('student.late_curfew.update', 'ID_PLACEHOLDER') }}">
    ...
    <div class="modal fade cmn-popwrp popwrp w400" id="deleteRequest" tabindex="-1" role="dialog"
        aria-labelledby="deleteRequest" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true"><img src="{{ asset('student/images/cross-icon.svg') }}"
...
@push('page_script')
    ...
    <script>
        document.getElementById('yearSelect').addEventListener('change', function() {
            if (this.value === '') {
                window.location.href = "{{ route('student.late_curfew_request') }}";
            }
        });
    </script>
@endpush
